...
// .pmem/transitions/T-login-submit-valid.json
{
  ...
  "tests":  ["TC101_正しい資格情報でログインできる"]  // 実テストを一意特定する文字列（grep で当たる）
}
```

- `Transition { id, action, given[], then[], tags?[], tests?[] }` — **これで全部**。
...
{ "id": "01J8XR...", "target": { "type": "transition", "id": "T-login-submit-valid" },
  ...
  "ref": "PR#42", "at": "2026-07-10T00:00:00Z" }
...
- `Decision { id, target: {type: "transition"|"tag", id}, why, changed?, ref?, at }` — **append-only**（消さない・直さない。訂正は新しい 1 件を足す）。
...
pmem diff [<gitref>]         # 現在 vs <gitref>（既定 HEAD）の semantic diff（語彙± / 遷移± / then 順序 / decisions±）
...
pmem lint [--json]
pmem diff [<gitref>] [--json]                             # 現在 vs ref の semantic diff（変更評価）
...
4. **完了ゲート（必須）** — 主題タグ単位で: マトリクス空白ゼロ／`tests` と実テストの双方向突合／穴探し 1 周。**lint 緑は網羅の証明ではない**。
