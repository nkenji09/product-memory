...
- `Transition { id, action, given[], then[], tags?[], priority? }` — **これで全部**。
  ...
  - **`priority`（#45 D8・`*int`・additive/omitempty・1 始まり・小さいほど先）— 同一 action 内の評価順の第一級宣言**。nil=未宣言（従来どおり）。同一 action 内でのみ意味を持ち、action をまたいだ比較は無意味。順序の正本欄が無いと実装ガイドが desc の散文に漏れて実装と食い違ったまま腐る（req.self-update の実証）ため、評価順を欄として持つ。負条件（else）は宣言から自動導出するので**否定条件は第一級化しない**（案F の半分だけ採用）。gap 検出への効き方は §3.4 と §8。
...
- `VocabEntry { id, category, label, kind?, owner?(effectのみ), tags?[], ref?, altLabels?[], establishes?[](effectのみ) }`
- **kind は 3 カテゴリすべてで任意**。設定値は config の宣言集合に含まれる必要がある（§5 `kind-valid`）。
- `owner` は effect の「その効果を起こす主体」（レイヤ／サービス名）。既定は任意の自由文（特定値を強制しない）。`config.ownerKind` を宣言すると owner は `kind==ownerKind` の subject タグ id 参照に構造化され write-time で実在検証される（オプトイン・#45 D9・§3.6）。owner の導出規則（遷移・action の owner 集合）と `multiple-owner-action` lint は §3.4。
- **★`tags`（tagId 参照）を持つ**。遷移は参照する語彙のタグを継承する（§3.7 実効タグ）。
- **関係スロット（#45 D5・additive/omitempty・既存レコードは無改修で読める）**:
  - `ref` — 外部契約・仕様本文へのアンカー（1行）。契約の全文は desc に散文で埋めず、versioned 文書（DESIGN の § 参照・OpenAPI 等の外部正本）を指す。`Tag.ref` との非対称を解消。file:line 形式は `ref-freshness` lint（warn）が警告する。
  - `altLabels` — 別表記・同義語。別の言い回しから既存語彙へ到達させ、重複新設を構造的に防ぐ。検索編入3面（CLI `scholia search`・viewer index 検索・viewer フィルタ/サジェスト）の検索対象。
  - `establishes`（effect のみ）— 「この効果が成立させる condition」の直接参照。値は現存 condition の id（書込時検証＋`vocab-ref` の参照整合 lint）。**transition 間の明示辺は持たない**という §2 の既決を維持したまま状態連鎖を機械可読にする（例: `eff.state.save-scroll-to-session` が `cond.view-scroll-in-session` を成立させる。`tx.viewer.scroll-save` の then と `tx.viewer.scroll-restore` の given が命名の察しでなく機械で繋がる）。図の導出本体は後フェーズ・本段は `show vocab`／VocabCard の双方向逆引き表示まで。
...
- **owner 導出規則と `multiple-owner-action` lint（#45 D9・info 級）**: `01KXNGQYS14G2XPDW19Y8JGX4B`（#40③）が「owner 導出設計を詰めてからの別提案」として保留した lint の実装。owner は effect の `VocabEntry.Owner`（`ownerKind` 宣言下では subject タグ id 参照・§3.6）にのみ在り、遷移・action の owner は**導出**する: **transition の owner 集合＝その `then` 効果の owner の和集合／action の owner 集合＝その action の全 transition の owner 集合の和集合**。`multiple-owner-action` lint は**宣言軸が張られた action のみ**（flow の `relevantAxes` 非空を再利用）を対象に、owner 集合が単一 subject に定まるかを検査する。複数 owner 混在・owner 無指定 effect の混在は **error にせず info で開示のみ**（偽 overlap の可能性・軸分析の単一 owner 前提を満たさない旨＋owner 列挙）。同一性は owner id の**厳密一致**（祖先畳み込みなし・必要な実データが出たら再訪）。`ownerKind` 未宣言時も走るが「自由文字列の完全一致ゆえ表記揺れで偽の複数判定が出うる」旨を finding に開示する。保存拒否・advisory ゲートには足さない（lint 走査のみ）。旗艦 `act.user.update` は全 then 効果が `subject.cli`（cli/update.go 実装）ゆえ owner 集合={subject.cli} 単一で沈黙し、宣言軸を持たない `act.user.flow` は検査対象外で沈黙する（#40③ の前提充足）。
- **評価順（`Transition.priority`）による重なり解決（#45 D8）— 保守的限定**: 同じ cell を取り合う重なり（overlap）／`Given(A)⊊Given(B)` の subset-shadow は、**そのグループの全 transition が相異なる priority を持つときだけ**「評価順で解決済み」に畳む（勝者＝最小 priority）。1つでも priority 未宣言 or 同 priority のペアが混じれば**従来どおり『優先順位未定義』として報告**する（本物の穴の置き場）。解決済みは既定表示から畳み、`--verbose` で derive した補集合（else）込みで開示する。
- **宣言的残余（declarative remainder）と L-total 免除（#45 D8/amend②）**: ある action の**全 transition に priority が宣言されている**とき、最後尾（最大 priority）を宣言的残余とみなし L-total を免除する（**部分宣言では免除しない**——署名なしの吸収で gap 検出を壊さないため）。宣言的残余は coverage に数えず別枠報告のまま。overlap 解決には参加させる（specifics から外さない）ので、旗艦 `act.user.update` は重なり11件が全件「解決済み」に畳まれ、L-total 0 になる。旧 `RemainderTagID` タグ規約は互換読み（そのタグ付き遷移も残余扱い・ただし L-total 免除は誘発しない）。
- **相対性の常時開示**: 評価順による解決は「宣言された priority に相対的で、実装の if/else 順との一致は検証していない」。CLI の scope-disclosure は常時この2行（評価順の相対性・宣言的残余は coverage 非算入）を印字し、viewer は一行 caveat の拡張で果たす（per-surface 原則・フル開示は CLI）。honesty-first を宣言層まで貫く（drift への答え・§8）。
...
### 3.5 意思決定（decision）— transition・tag・vocab のいずれにも付く
...
- **`commits[]`（git hash の集合・任意）は実装来歴**。`ref`（外部 URL/PR）とは別軸で、**repo 内の正確な着地点**を持つ
  ...
  - **append-only の精緻化（欄位単位・#45 U4→D7）**: decision の append-only とは「ファイル不変」ではなく
    ...
    「decision ファイル完全不変」から一段緩めた運用だが、CLI は追加しかできず、
...
- **容認の型付き宣言（#45 D6）**: 「意図的に残す gap」を機械可読に宣言する2系統。(a) `Decision.acknowledges[]` は容認する
  ...
  lint/flow の消費側は「**当該 target 宛て**の decision の acknowledges に該当 rule 名があれば finding を『容認済み
  （decision リンク付き）』に畳む」（baseline ratchet の新規 warn に数えない）。**祖先 decision では畳まない**——無関係な
  decision による偽陰性（信じられる lint が誤った緑を返す最悪の誤りモード）を作らないため。(b) `Tag.fulfillment`
  （""=transitions 既定・"property"）は「遷移では構造的に充足されない性質型要件」のマーカー。requirement-gap は
  fulfillment=property のタグを遷移充足検査から外すが、**property 宣言だけでは畳まない**——「当該タグ宛てで
  `acknowledges:[requirement-gap]` を含む decision が存在する」ときのみ緑（怠慢な宣言・偽陰性ガード）。
  flow finding の容認キーは〔rule × 対象（total-gap は axis タグ/欠落値 condition・subset-shadow/overlap は関与 transition）〕。
- **decision の読み面（#45 D10a）**: decision は横断到達（一覧・why 込み検索・共有可能な permalink）を持つ。viewer に
  `#/decisions` 一覧（target 種別・タグ・期間・現行/失効フィルタ）と `#/decision/<ulid>` 詳細（why 段落・changed 構造・
  commits hash チップ・ref リンク化・supersedes/superseded-by/acknowledges バッジ）を置き、検索コーパスに why/changed を
  編入し、static export にも同梱する。カード上の own-only 表示（既決 01KXDFD2RZJ118T2VVAF5F07RW）とはスコープが別
...
// .scholia/config.json
{
  "schemaVersion": 1,
  "kinds": {
    // condition は (string | object)[] の union（#45 D9）。object は
    // {id, label, description}（behaviors も可）。action/effect は string[] のまま。
    "condition": [
  ...
  "tagKinds":          ["requirement", "concern", "subject",       // string 宣言（縮退・id のみ）
                        { "id": "axis", "description": "網羅検査の軸", "behaviors": ["axis"] }],  // object 宣言
  "ownerKind":         "subject",                              // オプトイン（#45 D9）。宣言時 owner は subject タグ id 参照
...
## 5.5 表現域 — 「詳細設計で表現できるべきこと」の境界（#45 D5）

scholia が**担う**表現と、**担わず結線だけ持つ**表現の境界を正本化する。将来「これも表現したい」という要求が来たときの切り分け基準はここに照らす（勝手にスロットを増やさない）。

**判定原理**: (a) 遷移の分岐・順序・意図（＝変更評価の入力）に効くか → 担う。(b) 今の状態や外部正本から再生成・参照できるか → 担わず結線だけ持つ。補助基準: 到達性・一意性など「記録の運用」に効くもの（同義語・アンカー）は dogfood での破綻観測を条件に担う。

### 担う（11カテゴリ → 担当機構）

| # | 表現カテゴリ | 担当機構 |
|---|---|---|
| 1 | 分岐する振る舞い＝決定表 | 既存 transition（§3.2・変更なし） |
| 2 | 横断不変条件・要件トレーサビリティ | tag への decision（§3.5・cross-cutting） |
| 3 | 評価順序・競合解決 | priority（後フェーズ・P8） |
| 4 | 意図的な不在・容認・非機能要件 | acknowledges／property 要件（後フェーズ・P6） |
| 5 | 現行性——どの決定が今生きているか | supersedes 3値（後フェーズ・P7） |
| 6 | **状態の連鎖** | **`vocab.establishes`（#45 D5・§3.3）** |
| 7 | **語彙の why** | **vocab-target decision（#45 D5・§3.5）** |
| 8 | **外部契約への結線点** | **`vocab.ref`——アンカーのみ、本文は持たない（#45 D5・§3.3）** |
| 9 | **同義アクセス** | **`vocab.altLabels`（#45 D5・§3.3・検索編入3面）** |
| 10 | 適用スコープ——同じ挙動が複数ビューで起きること | 新スロットではなく**規約確定**: 「1原子＋複数 subject タグ」方式を正とし、複製方式は棚卸しで是正（下記） |
| 11 | 機械的意味論の宣言 | **kind の behaviors/description・ownerKind（#45 D9・実装済）**——kind 宣言を string｜object union へ／flow・lint の軸判定を behaviors 読取へ／ownerKind オプトイン＋write-time 検証／multiple-owner-action lint（§3.4・§3.6） |

### 担わない（8カテゴリ → 委ね先）

| 表現カテゴリ | 委ね先／理由 |
|---|---|
| ペイロード形状・値域 | OpenAPI/JSON Schema/CUE（`vocab.ref` で結線） |
| 境界値・例 | テスト層（`decision.commits` 経由・既決） |
| 投影 `view=f(state)` | 記録しない＝型検査の仕事（**A2 litmus を境界定義に編入**・下記） |
| 軸の排他・網羅の実世界真偽の機械検証 | しない（宣言・開示・derive 反映まで＝honesty-first 既決） |
| transition 間の明示辺 | 持たない（`establishes` からの導出で代替・§2 既決維持） |
| 時相性質の証明 | TLA+/Alloy の領域 |
| タグ間連想関係 | derive で足りる限り保留（「階層は1つ」既決） |
| 来歴（誰が・いつ） | git |

**A2 litmus（投影 vs 遷移の境界定義）**: 「その効果は今の状態からいつでも再生成できるか？」——**yes なら投影**（記録しない・型検査の仕事）、**no なら遷移**（記録する）。この判定はレコード内在情報からは機械判定できず、skill の意味判断が正位置（機械化しない・projection マーカー lint は「kind 誤分類が偽 advisory を生む二次誤りモード」として意図的に不採用）。

**「1原子＋複数 subject タグ」規約（カテゴリ10の確定・#45 D5）**: 同じ挙動が複数ビュー／コンポーネントで起きるとき、**1つの transition 原子に複数の subject タグを貼る**のを正とする（複製で同一原子を増やす方式は採らない）。`tx.viewer.scroll-save`/`scroll-restore` がこれを体現する。append-only では merge（共有化）は加算的でクリーン・split（分割）は履歴を汚す（concern.traceability 既決 01KXDFHK12JN0GPJ605B01WVGW の非対称原理）ため、複製を1原子へ統合する retrofit は「後からの意図的統合」として加算的にクリーンに行える。

### 非採用14件（表現域の外に置くと決めたもの・要旨）

hit policy 多値宣言／負条件の第一級化（priority が else を導出）／projection マーカー lint／config schemaVersion 2＋一括 migrate（string\|object 混在で代替）／自由 state id（`establishes` は conditionId 直接参照）／時刻比較型の鮮度検査／消費者の無い宣言の枠だけ切り（ordered・acknowledgedEmpty）／untyped 容認／フロー図 full derive（後フェーズ維持）／タグ間連想関係／契約の構造化スロット（`ref` アンカーのみ）／litmus の lint 化／lint error 昇格の全面化／CLI と Viewer の rules「統一」（own 保持＋並置＋amend で解く）。この非採用リスト自体を表現域の正本として残す（将来「なぜ足さないのか」を辿れるように）。

### 5.5.1 scope-disclosure 契約（#45 U4・`eff.emit.scope-disclosure` の `ref` アンカー先）

`eff.emit.scope-disclosure` は flow/gaps の解析結果に「保証の外」を必ず印字する開示 emit。この契約の全文はここを正本とし、vocab の desc には散文で埋めず `vocab.ref` でこの節を指す（契約文書ルートを増やさず DESIGN を第2正本とする）。

- flow/gaps は素の「no gaps」を出さない。解析が触れていない領域（列挙した軸・don't-care 条件・未宣言軸・default 残余）を必ず「保証の外」として開示する。
- 軸が0件のときは (a) その action に axis タグが未宣言なのか、(b) この action に軸が効いていないのか、を**区別して**開示する（黙って「gap なし」に見せない）。
- surface ごとに粒度を変えて果たしてよい（既決 per-surface 01KXMGH6N6HTDVYFT9JBK6A7S9）: CLI 経路はフルの scope-disclosure、viewer は一行 caveat で果たす。確定判断の来歴は decision 01KXNGQYRM718XH18RGSNSQCW1（#40①）。
...
scholia config infer-id-policy [--json]                       # 既存 id 分布から idPolicy 宣言案を提示（read-only・#45 U2）

# 語彙
scholia vocab add <condition|action|effect> <id> --label <l> [--kind <k>] [--owner <l>] [--ref <a>] [--alt-label <t>]… [--establishes <condId>]…  # ref/altLabels/establishes は #45 D5（establishes は effect のみ・実在検証）。--owner は ownerKind 宣言下では実在 subject タグ id を検証（#45 D9）
scholia vocab edit <id> [--label <l>] [--kind <k>] [--owner <l>] [--description <d>|--desc-file <f>|--edit] [--ref <a>] [--alt-label <t>]… [--clear-alt-labels] [--establishes <condId>]… [--clear-establishes]  # --label は空不可。--kind は kind backfill（宣言集合検査）・--owner は effect のみ＋ownerKind 検証（#45 D9）
scholia vocab owner-migrate [--json]                          # 既存 owner 自由文字列 → subject タグへのマッピング案を提示（書き込みなし・#45 D9）
...
# 遷移（原子）
scholia tx add <id> --action <a> [--given <c,…>] --then <e,…> [--tags <t,…>] [--priority <n>]  # priority は同一 action 内の評価順（1始まり・小さいほど先・未指定＝未宣言・#45 D8）
scholia tx edit <id> [--action][--given][--then][--tags][--priority <n>|--clear-priority]       # --priority は1以上必須／--clear-priority で未宣言へ戻す（#45 D8）
