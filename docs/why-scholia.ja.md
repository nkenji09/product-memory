...
新しい decision は、置き換える旧 decision を `supersedes` で名指しできる。
だから「今どれが正か」を記録の読み比べや日付の新旧から推測する必要はなく、リンクを辿って機械的に判定できる。
...
- **意図的な gap と見落としを区別できる**：lint や flow が検出した gap が「decide で容認済み」なのか「本当に見落としなのか」を、rule id 単位の紐付け（`acknowledges`）で機械的に判別できる。祖先タグの decision では畳まれないので、無関係な decision が別の gap を偽って隠すこともない。
...
タグや語彙や遷移の ID は、`req.components-formInputs-widgeta` や `act.api.clear` のようなドット名前空間のスラグにする。
...
`req.components-formInputs-widgeta` のようなスラグは、区切りが実質の「仮想フォルダ」として働く。
