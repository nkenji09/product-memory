...
3. **B・D のときは合意の重さも判定する**——観測可能な振る舞い（UI・CLI 出力・操作結果など人が使って気づく差）を
   変えるか否かで、着地時に必要な対話の重さが決まる（scholia-change の adopt 手順が消費する）:
   - **重い** — 観測可能な振る舞いが変わる。adopt 前に変更前/変更後を1項目ずつ対話で説明する必要がある。
   - **軽い** — spec の記述精度・言い回しのみで、観測可能な振る舞いは変えない（doc-fidelity・精緻化）。
     サマリー＋Adopt で足りる。
   迷ったら重い側に倒す（軽いと判定して対話を省く方が、事故の被害が大きい）。

4. **方針を出力して手を止める**——次の 4 点を出し、**何も変更しない**:
