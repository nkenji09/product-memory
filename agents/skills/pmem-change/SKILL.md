...
**Phase 1（viewer 非依存の最小形）**: viewer にはまだ decision を書き込む経路が無い。UI のコメントドロワー、
または端末でコメントを集め、**コピーして本スキルを AI に貼り付ける**ところから始まる。以降は既存 CLI
（`diff`/`rules`/`list`/`decide`/`decision add-commit`）とコメントの copy-paste だけで完結する
（viewer 書込・diff 可視化などは後続フェーズの設計で本スキルの範囲外）。
...
5. **提案 diff を出す**:
   ...
   ```
6. **人がレビュー** — UI／端末で diff とコメントを見比べる。調整があれば手順 4〜5 に戻り `.pmem/` を編集し直す。
...
3. **`.pmem/` を編集** — transition（`given`/`then`/`tags`/`test` 等）を変更する。
4. **提案 diff を出す** — `pmem diff`（作業ツリー vs base）。
5. **人がレビュー** — 提案・コメントを見比べる。調整があれば手順 3〜4 に戻る。
...
- **adopt** — ブランチを merge（人が行う）＋「採用」の decision を `pmem decide` で append。手順は
  Case 1/2 の 7〜9 の通り。
- **reject** — merge しない ＋「取り込まない・理由」の decision を append する。一言で済ませず、
...
- `pmem diff` の ref 対 ref 拡張・CLI 全体は `DESIGN.md` §6 が正本（`pmem <cmd> --help` も真値）。
- viewer 書込・diff 可視化・task の UI 化など後続フェーズの設計は本スキルの範囲外。
