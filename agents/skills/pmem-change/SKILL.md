...
着手する前に、正本の変更（decision 下書き＋提案）を人が `pmem view` の深リンクでレビュー・承認する
ゲートを必ず挟む。**設計メモ（作業ツリーの散文ドラフト等）のレビューは正本レビューの代替にならない**
——正本＝pmem の decision/transition でレビューする。decision の `ref` は**永続物**（commit・DESIGN 節・
...
5. **提案 diff を出す**:
   ...
   ```
6. **人がレビュー** — AI は対象タグ／transition を開いた `pmem view` の深リンク URL を渡し、人はそこで
   ドロワー（差分カード＋提案コメント）を見比べる。調整があれば手順 4〜5 に戻り `.pmem/` を編集し直す
...
5. **人がレビュー** — AI は対象 transition を開いた `pmem view` の深リンク URL を渡し、人はそこで
...
- prospective の場合、実装ワーカー／コード実装に着手する前に、正本の変更を `pmem view` の深リンクで
