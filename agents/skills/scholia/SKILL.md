...
- [ ] **軸は action から見つけたか** — enum 型フィールドを一律 axis にしない。値で action の観測可能な結果が変わる状態だけが軸。判定は「その効果は今の状態からいつでも再生成できるか？」＝ **YES は投影**（型検査の仕事・軸にしない）／**NO は遷移**（軸候補）（§3）。
- [ ] **きっかけ・前提・結果を書き分けたか** — condition に結果を混ぜない（結果は `then`／effect の責務・§4）。
- [ ] **desc は「これは何か」だけか** — why・値数の理由・`total` の是非・owner は decision へ。desc に二重書きしない（§5）。
- [ ] **markdown を使ったか** — prop は `` `code` ``・強調は `**bold**`・長文ベタ書きにしない（§4）。
- [ ] **派生できる情報を書き写していないか** — 軸の値一覧など、構造から導出できるものを desc / name に列挙しない（§6）。
- [ ] **「〜を参照」というメタ指示を書いていないか** — 根拠は構造〔タグ → decision〕で辿れる。散文の道案内を desc に埋めない（§5）。
